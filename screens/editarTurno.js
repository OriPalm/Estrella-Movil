    ...
    const cargarTurno = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          const turnoFecha = DateTime.fromISO(data.fechaTurno).set({
            hour: data.horaTurno.split(':')[0],
            minute: data.horaTurno.split(':')[1],
          });
  
          const now = DateTime.local();  // Obtener la fecha y hora actual
  
          // Verificar si el turno es pasado solo si la fecha y la hora están en el pasado
          if (turnoFecha < now) {
            setIsEditable(false); 
  ...
  const verificarTurnoExistente = async () => {
    try {
      ...
      const q = query(
        ...
        where('__name__', '!=', turnoId) // Excluye el turno actual usando su ID
      );
